...
#define HIGHTIME 63.3 // number of 48MHz cycles to be high for 1.65uS

// setup Timer2 for 48MHz, and setup the output pin B6 
void ws2812b_setup() {
    ...
    LATBbits.LATB6 = 0; // LAT...
    
//    while (_CP0_GET_COUNT()<(48000000/2)/1){}   //1hz
//    _CP0_SET_COUNT(0);
//        TMR2 = 0;
//    
...
// c is in the order of LED0 - LED n
void ws2812b_setColor(wsColor * c, int numLEDs) {
    ...
    // loop through each WS2812B, r value 
    for (i = 0; i < numLEDs; i++) {
...
//    unsigned char iblue = blue * 255.0;
    ...
    unsigned char ired = 122.0;
    unsigned char igreen = 0.0;
    unsigned char iblue = 0.0;
    wsColor c;
    c.r = ired;
    c.g = igreen;
    c.b = iblue;
